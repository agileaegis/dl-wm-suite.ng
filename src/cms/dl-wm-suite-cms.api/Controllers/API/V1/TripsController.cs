...
using dl.wm.suite.cms.api.Redis.Models.TrackingPoints;
...
namespace dl.wm.suite.cms.api.Controllers.API.V1
{
  ...
  public class TripsController : BaseController
  {
    ...
    public async Task<IActionResult> GetTripAsync(Guid id, [FromQuery] string fields)
    {
        ...
        [FromQuery] TripsResourceParameters tripsResourceParameters, [FromHeader(Name = "Accept")] string mediaType)
    {
      ...
      if (mediaType.Contains("application/vnd.marvin.hateoas+json"))
      {
        ...
        var shapedTripsWithLinks = shapedTrips.Select(trip =>
        {
          var tripAsDictionary = trip as IDictionary<string, object>;
          var tripLinks =
              CreateLinksForTrip((Guid)tripAsDictionary["Id"],
                  tripsResourceParameters.Fields);

          tripAsDictionary.Add("links", tripLinks);
    ...
    public async Task<IActionResult> PostPointToTrackableAsync(Guid id, [FromBody] TrackingPointDto point)
    {
      var userAudit = GetEmailFromClaims();
    ...
    private IEnumerable<LinkDto> CreateLinksForTrip(Guid id, string fields)
    {
