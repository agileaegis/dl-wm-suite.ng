...
namespace dl.wm.suite.cms.services.Trips
{
    public class UpdateTripProcessor : IUpdateTripProcessor
    {
        ...
        private readonly IVehicleRepository _assetRepository;
        private readonly ITrackableRepository _trackableRepository; 
        private readonly IAutoMapper _autoMapper;

        public UpdateTripProcessor(IUnitOfWork uOf, IAutoMapper autoMapper, ITripRepository tripRepository, IVehicleRepository assetRepository, ITrackableRepository trackableRepository)
        {
            _uOf = uOf;
        ...
        public Task<TripUiModel> UpdateTripAsync(string accountEmailToUpdateThisTrip, TripForModificationUiModel updatedTrip)
        {
            ...
            try
            {
                ...
                var assetToBeInjected = _assetRepository.FindByNumPlate(updatedTrip.TripAssetNumPlate);
        ...
        public Task<TripUiModel> CreateTrtackingPoint(string accountEmailToUpdateThisTrip, Guid tripId,
            TrackingPointDto point)
        {
            ...
            if (tripId == Guid.Empty || point == null)
            {
                response.Message = "ERROR_INVALID_TRIP_MODEL";
                return Task.Run(() => response);
            }
